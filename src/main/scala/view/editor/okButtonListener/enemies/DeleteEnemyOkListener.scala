...
case class DeleteEnemyOkListener(override val form: Form,
  ...
  override def editorControllerAction(): Unit = controller.deleteEnemyFromNode(form.elements.head.value.toInt)

  override def inputConditions: List[(Boolean, String)] = List(
    (form.elements.head.value != null, mustBeSpecified(TheId))
  )
